...
# Resolve private-registry credentials too, only if REGISTRY_URL is set.
if [ -n "$REGISTRY_URL" ]; then
  REGISTRY_USERNAME=$(resolve REGISTRY_USERNAME)
  REGISTRY_PASSWORD=$(resolve REGISTRY_PASSWORD)
  if [ -z "$REGISTRY_USERNAME" ] || [ -z "$REGISTRY_PASSWORD" ]; then
    echo "Error: REGISTRY_URL is set but REGISTRY_USERNAME / REGISTRY_PASSWORD could not be resolved (set directly, or store as Infisical secrets of the same name)."
    exit 1
  fi
fi
